...
.wealth-title { font-size: 2rem; font-weight: 800; color: #e5e7eb; margin-bottom: 0.20rem; }
.wealth-subtitle { color: #cbd5e1; margin-bottom: 1.15rem; }
.decision-good { color: #10b981; font-weight: 800; font-size: 1.1rem; }
.decision-warn { color: #f59e0b; font-weight: 800; font-size: 1.1rem; }
.decision-bad { color: #ef4444; font-weight: 800; font-size: 1.1rem; }
.decision-wait { color: #94a3b8; font-weight: 800; font-size: 1.1rem; }
...
def guide_rows() -> List[Tuple[str, str, str]]:
    return [
        ("Compound Profit %", "الربح المركب", "نسبة الربح عند إعادة استثمار نتيجة كل صفقة فوق التي قبلها."),
        ("Strategy P/L", "ربح الاستراتيجية الأصلي", "نتيجة الباكتيست حسب منطق وحجم الصفقة داخل الاستراتيجية."),
        ("Max Drawdown", "أكبر هبوط", "أكبر تراجع من قمة رأس المال إلى قاع لاحق."),
        ("Win Rate", "نسبة الربح", "عدد الصفقات الرابحة ÷ إجمالي الصفقات."),
        ("Profit Factor", "عامل الربح", "إجمالي الأرباح ÷ إجمالي الخسائر."),
        ("Sharpe", "جودة العائد", "مقياس العائد مقابل التذبذب."),
        ("Has Open Trade", "هل توجد صفقة مفتوحة", "YES يعني الاستراتيجية لديها مركز مفتوح الآن."),
        ("Smart Decision", "القرار الذكي", "يوضح وضع الصفقة: ربحانة، خسرانة فوق الستوب، قريبة من الستوب، انتظار..."),
        ("Current Stop", "الستوب الحالي", "مستوى الخروج الوقائي الحالي حسب الاستراتيجية."),
        ("Current Target", "الهدف الحالي", "مستوى جني الربح الحالي حسب الاستراتيجية."),
        ("Risk to Stop %", "المسافة إلى الستوب", "كم تبعد الصفقة عن الستوب بالنسبة المئوية."),
        ("Reward to Target %", "المسافة إلى الهدف", "كم تبعد الصفقة عن الهدف بالنسبة المئوية."),
        ("Recent Events", "آخر الأحداث", "آخر صفقتين مغلقتين + الصفقة المفتوحة الحالية إن وجدت."),
    ]
...
def ensure_excel_guide_sheet(path: Path) -> None:
    ...
    try:
        ...
        if "Guide" not in wb.sheetnames:
            ...
            ws.append(["Field / Column", "المعنى", "شرح بسيط"])
...
def plan_run_args(plan_file: Path, fallback_years: int = 1) -> List[str]:
...
def run_signal_for_plan(plan_file: Path, years: int = 1, adjust: str = "splits") -> Dict[str, Optional[Path]]:
    symbol = symbol_from_json(plan_file)
    start_ts = datetime.now().timestamp()
    rc, log = run_core_command_stream([
        symbol, "--mode", "signal",
        *plan_run_args(plan_file, fallback_years=years),
        "--adjust", adjust,
...
def load_watchlist() -> List[str]:
    if not WATCHLIST_FILE.exists():
        return []
    data = safe_load_json(WATCHLIST_FILE)
...
def save_watchlist(symbols: List[str]) -> None:
    ...
    with open(WATCHLIST_FILE, "w", encoding="utf-8") as f:
        json.dump({"symbols": symbols, "updated_at": datetime.now().isoformat(timespec="seconds")}, f, indent=2)
...
def risk_filter_for_signal(sig: Dict[str, Any], summary: Dict[str, Any]) -> Dict[str, str]:
    ...
    if not has_open:
        return {
            "label": "NO_OPEN_TRADE",
            "level": "wait",
            "message": "لا توجد صفقة مفتوحة حاليًا. القرار متابعة وانتظار إشارة جديدة.",
        }

    pnl = sig.get("current_pnl_pct")
    risk = sig.get("risk_to_stop_pct")
    reward = sig.get("reward_to_target_pct")
    ...
    if "NEAR_STOP" in action or (risk_f is not None and abs(risk_f) <= 2.0):
        return {
            ...
            "level": "bad",
            "message": "الصفقة قريبة من الستوب. لا تضيف على الصفقة وراقب الخروج.",
        }
    ...
    if pnl_f is not None and pnl_f < -3:
        return {
            ...
            "level": "warn",
            "message": "الصفقة خاسرة حاليًا. الالتزام بالستوب أهم من التعديل العشوائي.",
        }
    ...
    if reward_f is not None and reward_f <= 3 and pnl_f is not None and pnl_f > 0:
        return {
            "label": "NEAR_TARGET",
            "level": "good",
            "message": "الصفقة قريبة من الهدف. الأفضل متابعة جني الربح أو الستوب المتحرك حسب الخطة.",
        }
    ...
    if pnl_f is not None and pnl_f >= 5:
        return {
            "label": "HOLD_PROFIT",
            "level": "good",
            "message": "الصفقة رابحة. استمر بالمتابعة ولا تخرج قبل تحقق سبب واضح.",
        }

    return {
        "label": "NORMAL_RISK",
        "level": "wait",
        "message": "المخاطرة طبيعية حاليًا حسب البيانات المتاحة. التزم بالخطة.",
    }
...
def main() -> int:
    ...
    try:
        ...
        if rc != 0:
            update("failed", phase="optimize", returncode=rc, error="Optimize failed. افتح اللوج لمعرفة السبب.")
            return rc
        ...
        if sig_rc != 0:
            update(
                "done",
                phase="signal_failed",
                warning="Optimize completed, but Signal failed. النتيجة محفوظة ويمكنك مراجعة اللوج.",
...
def render_background_job_status() -> Optional[Dict[str, Any]]:
    ...
    if status == "running" and job.get("pid") and not pid_is_running(job.get("pid")):
        ...
        if str(refreshed.get("status", "")).lower() == "running":
            ...
            refreshed["error"] = "Background process stopped before writing a final result. افتح اللوج لمعرفة السبب."
    ...
    if status in ("running", "starting"):
        ...
        st.info(f"Analysis still running in background: {symbol} / phase={phase}. تقدر تروح على أي صفحة وترجع بدون ما ينقطع التحليل.")
        st.progress(progress, text=f"Running {symbol}... {progress * 100:.0f}%")
    ...
    if status == "done":
        ...
        st.success(f"Background analysis completed for {job.get('symbol', '-')}.")
        if job.get("warning"):
...
def page_analyze_stock() -> None:
    ...
    st.markdown('<div class="wealth-subtitle">حلل سهم جديد بتاريخ بداية ونهاية ومبلغ استثمار. التحليل يعمل بالخلفية، وتقدر تنتقل بين الصفحات بدون ما ينقطع.</div>', unsafe_allow_html=True)
    ...
    if start_date >= end_date:
        st.error("Start Date لازم يكون قبل End Date.")
        return
    ...
    if last and result_inputs_differ_from_form(last, form_inputs):
        ...
        st.warning(
            "النتيجة المعروضة بالأسفل تخص: "
            f"{inputs_for_display(old_inputs)}. "
            f"الفورم الحالي: {inputs_for_display(form_inputs)}. "
            "اضغط Analyze Stock حتى تطلع نتيجة جديدة للفورم الحالي."
        )
    ...
    if st.button("🚀 Analyze Stock", type="primary", use_container_width=True, disabled=analyze_disabled):
        ...
        try:
            ...
            st.success("Started background analysis. تقدر تنتقل بين الصفحات وترجع تشوف الحالة.")
            st.rerun()
        except Exception as exc:
    ...
    if analyze_disabled:
        st.caption("في تحليل شغال الآن. إذا بدك تبدأ تحليل جديد، انتظر يخلص أو اضغط Cancel Job.")
    ...
    if last:
        ...
        st.success(f"Analysis completed for {inputs_for_display(result_inputs)}. هذه نتيجة مؤقتة؛ لن تظهر في الصفحات الأخرى إلا بعد Save as Active Plan.")
        ...
        if sig:
            ...
            st.write(sig.get("decision_reason") or sig.get("notes") or "")
            ...
            oc6.metric("Risk to Stop", fmt_signed_pct(sig.get("risk_to_stop_pct")))
            oc7.metric("Reward to Target", fmt_signed_pct(sig.get("reward_to_target_pct")))
        ...
        with a1:
            if st.button(f"⭐ Save {symbol} as Active Plan", use_container_width=True):
                saved = save_active_outputs_from_last(symbol, last)
                ...
                last["opt_json"] = str(report_json)
                last["opt_xlsx"] = str(saved["OPTIMIZE"].get("xlsx") or last.get("opt_xlsx") or "")
                last["signal_json"] = str(saved["SIGNAL"].get("json") or last.get("signal_json") or "")
                last["signal_xlsx"] = str(saved["SIGNAL"].get("xlsx") or last.get("signal_xlsx") or "")
                last["package"] = str(package_path)
                last["saved_plan"] = str(plan_path)

                st.session_state["last_analysis"] = last
                persist_last_analysis(last)
                st.session_state["last_saved_plan"] = str(plan_path)

                st.success(f"Saved active plan: {plan_path.name}")
                st.info(f"Official reports saved to reports/ and package updated: {package_path.name}")
        ...
        with a2:
            ...
            if package and package.exists() and package.is_file():
                ...
                st.caption("Package appears after Save as Active Plan.")

        with a3:
            if last.get("opt_xlsx"):
                download_button_for_file(Path(last["opt_xlsx"]), "Download Optimize Excel", key_prefix="analyze_excel")

def page_dashboard() -> None:
    st.markdown('<div class="wealth-title">Dashboard</div>', unsafe_allow_html=True)
    st.markdown('<div class="wealth-subtitle">ملخص أداء الخطط المحفوظة فقط.</div>', unsafe_allow_html=True)

    param_files = active_param_files()
    if not param_files:
        st.info("No active plans yet. حلل السهم من Analyze Stock، وبعدها اضغط Save as Active Plan حتى يظهر هنا.")
        return

    raw_df = pd.DataFrame([dashboard_row(p) for p in param_files])

    # Dashboard = performance summary only.
    ...
    with col1:
        years = st.number_input("Refresh years", min_value=1, max_value=20, value=int(st.session_state.get("auto_refresh_years", 1)), step=1)
        st.session_state["auto_refresh_years"] = int(years)
    with col2:
        if st.button("🔄 Refresh All Signals", type="primary", use_container_width=True):
            for p in param_files:
                run_signal_for_plan(p, years=int(years))
            st.success("Signals refreshed.")
            st.rerun()
    with col3:
        download_dataframe_csv(df, "dashboard_table.csv", "⬇️ Download Table CSV", "download_dashboard_csv")

    with st.expander("Auto Refresh", expanded=False):
        st.caption("يحدث الإشارات للأسهم المحفوظة تلقائيًا. انتبه لاستهلاك API.")
        auto_enabled = st.toggle("Enable Auto Refresh", value=bool(st.session_state.get("auto_refresh_enabled", False)))
        interval = st.number_input("Every X minutes", min_value=5, max_value=240, value=int(st.session_state.get("auto_refresh_interval", 30)), step=5)
        st.session_state["auto_refresh_enabled"] = auto_enabled
        st.session_state["auto_refresh_interval"] = int(interval)
        if auto_enabled:
            st.success(f"Auto Refresh ON: every {int(interval)} minutes.")
            run_auto_refresh_if_due(int(interval))
        else:
            st.info("Auto Refresh OFF.")

    st.markdown("### Active Plans Summary")
    st.dataframe(style_dashboard_df(df), use_container_width=True, hide_index=True)

def page_trades_overview() -> None:
    st.markdown('<div class="wealth-title">Trades Overview</div>', unsafe_allow_html=True)
    st.markdown('<div class="wealth-subtitle">متابعة حالة الصفقات الحالية للأسهم المحفوظة فقط.</div>', unsafe_allow_html=True)

    symbols = active_plan_symbols()
    if not symbols:
        st.info("No active plans yet. أي سهم لن يظهر هنا إلا بعد Save as Active Plan.")
        return
    ...
    for symbol in symbols:
        ...
        rows.append({
            ...
            "Risk to Stop %": fmt_signed_pct(sig.get("risk_to_stop_pct")) if has_open else "-",
            "Reward to Target %": fmt_signed_pct(sig.get("reward_to_target_pct")) if has_open else "-",
    ...
    if missing_signal:
        st.warning("No Signal file yet for: " + ", ".join(missing_signal) + ". Use Dashboard > Refresh All Signals or Signal Detail > Refresh Signal.")
    ...
    with dcol2:
        download_dataframe_csv(df, "trades_overview_table.csv", "⬇️ Download Table CSV", "download_trades_overview_csv")

    st.dataframe(style_dashboard_df(df), use_container_width=True, hide_index=True)

    st.caption("آخر صفقتين والتفاصيل الكاملة موجودة في صفحة Signal Detail حتى ما نكرر المعلومات هنا.")

def page_signal_detail() -> None:
    st.markdown('<div class="wealth-title">Signal Detail</div>', unsafe_allow_html=True)
    st.markdown('<div class="wealth-subtitle">تفاصيل السهم الواحد: القرار، السبب، الدخول، الستوب، الهدف، وآخر صفقتين.</div>', unsafe_allow_html=True)
    symbols = active_plan_symbols()
    if not symbols:
        st.info("No active plans yet. أي سهم لن يظهر هنا إلا بعد Save as Active Plan.")
        return
    ...
    if not plan_file:
        st.warning("No usable JSON found for this symbol.")
        return
    col1, col2, col3 = st.columns([1, 2, 2])
    with col1:
        years = st.number_input("Years", min_value=1, max_value=20, value=1, step=1, key="detail_years")
    with col2:
        if st.button("🔄 Refresh Signal", type="primary", use_container_width=True):
            run_signal_for_plan(plan_file, years=int(years))
            st.rerun()
    with col3:
        if st.button("▶️ Run Replay", use_container_width=True):
            run_replay_for_plan(plan_file, years=int(years))
            st.rerun()
    payload = extract_signal_payload(symbol)
    if not payload:
        st.warning("No Signal saved yet. Click Refresh Signal. إذا ضغطت Refresh وما ظهر، افتح Signal Log لمعرفة الخطأ.")
        return
    summary = payload.get("summary", {})
    sig = payload.get("current_signal", {})
    events = payload.get("recent_events", [])
    decision = sig.get("action", "-")
    st.markdown(f"### {symbol} / {summary.get('timeframe') or timeframe_from_payload(payload)}")
    st.markdown(f'<div class="{decision_class(decision)}">Smart Decision: {decision}</div>', unsafe_allow_html=True)
    st.write(sig.get("decision_reason") or sig.get("notes") or "")
    c1, c2, c3, c4 = st.columns(4)
    ...
    c6.metric("Risk to Stop", fmt_signed_pct(sig.get("risk_to_stop_pct")))
    c7.metric("Reward to Target", fmt_signed_pct(sig.get("reward_to_target_pct")))
    ...
    if package and package.exists() and package.is_file():
        download_button_for_file(package, f"Download {package.name}", key_prefix="signal_package")
...
def page_files_reports() -> None:
    ...
    st.markdown('<div class="wealth-subtitle">ملفات الأسهم المحفوظة فقط، مع حذف آمن عبر Archive.</div>', unsafe_allow_html=True)
    ...
    if status_row.get("Plan") == "Missing":
        st.warning(f"Plan is missing. اضغط Save as Active Plan من صفحة Analyze حتى يدخل ملف {symbol}_PLAN.json داخل الحزمة.")

    package = latest_package(symbol)
    if package and package.exists() and package.is_file():
        st.subheader("Download Package")
        st.write(f"Package file: `{package.name}`")
        st.caption("Inside ZIP: SYMBOL_PLAN.json, SYMBOL_OPTIMIZE.xlsx/json, SYMBOL_SIGNAL.xlsx/json, SYMBOL_REPLAY.xlsx/json")
    ...
    st.warning("Remove Active Plan ينقل ملفات السهم إلى archive/ ولا يحذفها نهائيًا.")
    confirm = st.text_input(f"Type {symbol} to confirm removal", key=f"remove_confirm_{symbol}")
    if st.button(f"🗄️ Archive / Remove {symbol} Active Plan", type="secondary", disabled=(confirm.strip().upper() != symbol)):
...
def page_auto_refresh() -> None:
    ...
    st.markdown('<div class="wealth-subtitle">تحديث Signal تلقائيًا للأسهم المحفوظة كـ Active Plan فقط.</div>', unsafe_allow_html=True)

    active_symbols = set(active_plan_symbols())
    active_count = len(active_symbols)

    st.metric("Active Plans Covered", active_count)

    st.caption("Auto Refresh لا يعمل Optimize ولا يضيف أسهم جديدة. هو فقط يحدث نقاط الدخول والخروج والقرار للأسهم المحفوظة في plans/.")

    enabled = st.toggle("Enable Auto Refresh", value=bool(st.session_state.get("auto_refresh_enabled", False)), key="auto_enabled")
    ...
    years = st.number_input("Signal years", min_value=1, max_value=20, value=int(st.session_state.get("auto_refresh_years", 1)), step=1, key="auto_years")
    ...
    if enabled:
        st.success(f"Auto Refresh ON every {int(interval)} minutes for saved active plans only.")
        run_auto_refresh_if_due(int(interval))
    else:
        st.info("Auto Refresh OFF.")
    ...
    if active_symbols:
        ...
        st.dataframe(pd.DataFrame({"Symbol": sorted(active_symbols)}), use_container_width=True, hide_index=True)
    else:
        st.warning("No active plans yet. احفظ سهم كـ Active Plan أولًا.")

    if st.button("🔄 Refresh Active Signals Now", type="primary"):
        for p in active_param_files():
            run_signal_for_plan(p, years=int(years))
        st.success("Active signals refreshed.")
        st.rerun()
...
def main() -> None:
    ...
    core = find_core_script()
    if core:
        st.sidebar.success(f"Core: {core.name}")
    else:
        st.sidebar.error("Core script not found")
        st.sidebar.caption("Expected: Wealth Engine.py")
    page = st.sidebar.radio("Navigation", ["Analyze Stock", "Dashboard", "Trades Overview", "Signal Detail", "Files / Reports", "Auto Refresh"])
    st.sidebar.markdown("---")
    st.sidebar.caption("Plans folder: plans/")
    st.sidebar.caption("Reports folder: reports/")
    st.sidebar.caption("Packages folder: packages/")
    if page == "Analyze Stock":
        page_analyze_stock()
    elif page == "Dashboard":
        page_dashboard()
    elif page == "Trades Overview":
        page_trades_overview()
